...
class EditjadwalPage extends StatefulWidget {
  ...
  const EditjadwalPage({super.key, required this.anggota});
...
class _EditjadwalPageState extends State<EditjadwalPage> {
  ...
  late TextEditingController nama_acara;
  late TextEditingController nama_tempat;
  ...
  void initState() {
    ...
    nama_acara = TextEditingController(
      text: widget.anggota['nama_acara']?.toString() ?? '',
    );
    nama_tempat = TextEditingController(
      text: widget.anggota['nama_tempat']?.toString() ?? '',
    );
    waktu = TextEditingController(
      text: widget.anggota['waktu']?.toString() ?? '',
    );
    alamat = TextEditingController(
      text: widget.anggota['alamat']?.toString() ?? '',
    );
  ...
  Future<void> saveDelete() async {
    final token = await getToken();
    final url = Uri.parse("${GlobalConst.url}/api/v1/jadwal/${id.text}");
  ...
  Future<void> saveEdit() async {
    ...
    final payload = {
      ...
      "nama_acara": nama_acara.text,
      "nama_tempat": nama_tempat.text,
      "waktu": waktu.text,
      "alamat": alamat.text,
    };
    print(payload);
  ...
  Future<void> saveNew() async {
    ...
    final token = await getToken();
    final url = Uri.parse("${GlobalConst.url}/api/v1/jadwal/");
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            CustomTextField(
              controller: nama_acara,
              label: "Nama Acara",
              icon: Icons.person,
            ),
            const SizedBox(height: 16),
            CustomTextField(
              controller: nama_tempat,
              label: "Nama Tempat",
              icon: Icons.person,
            ),
            const SizedBox(height: 16),
            ...
            DatePickerField(
              label: "Tanggal",
              controller: waktu, // isinya langsung yyyy-MM-dd
            ),
            const SizedBox(height: 24),
            Row(
