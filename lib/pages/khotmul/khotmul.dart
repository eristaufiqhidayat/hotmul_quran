...
import 'package:hotmul_quran/model/jumlah_ayat.dart';
...
class _KhotmulPageState extends State<KhotmulPage> {
  ...
  var juzNumber;
  JuzData? totalAyah; // nanti bisa diganti sesuai data asli per juz
  Set<int> readAyahs = {};
  ...
  Future<void> ayahCount({String? juz}) async {
    if (!mounted) return;
    setState(() => isLoading = true);
    ...
    final url = Uri.parse("${GlobalConst.url}/api/v1/juz/$juz");
    ...
    if (response.statusCode == 200) {
      final result = json.decode(response.body);
      final List<JuzData> dataList = (result['data'] as List)
          .map((e) => JuzData.fromJson(e))
          .toList();
      setState(() {
        totalAyah = dataList.first;
      });
    }
